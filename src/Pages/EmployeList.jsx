...
const filterData = employees.filter(item=>item.name.toLocaleLowerCase().includes(searchQuery.toLocaleLowerCase()))
  return (
    <div className="p-2 sm:p-4 ">
      {/* Top Actions */}
      <div className="flex flex-col sm:flex-row justify-between gap-3 mt-2 mb-8 w-full  ">
        {/* Search Box
        ...
        <h1 className="text-2xl sm:text-2xl font-semibold text-[#252C58] dark:text-gray-50 ">
          Employee List
        </h1>

        {/* Buttons */}
        <div className="flex gap-2 sm:gap-3">
          <div className="bg-[#9376CA] text-white text-[0.7rem] px-3 rounded-sm flex items-center gap-1 py-1 ">
            <Link to="/info" className="flex items-center gap-1 text-[0.8rem]">
              <AiOutlinePlus className="w-4 h-4 font-semibold" />
              Add Employee
            </Link>
          </div>
        </div>
      </div>
      <div className="flex justify-around gap-2 w-full flex-wrap md:flex-nowrap">
        <div className="flex gap-2  rounded-sm px-3 items-center shadow drop-shadow-xs border border-gray-300 dark:border-gray-500 dark:bg-gray-800  w-[70%]  xl:h-[35px]">
          <input
            type="text"
            placeholder="Search here"
                    onChange={(e) => setSearchQuery(e.target.value)}

            className="px-3 py-2 w-full  text-[0.8rem] outline-none"
          />
        ...
        <button
          ...
          className="bg-[#9376CA] dark:bg-gray-800 dark:border dark:border-gray-500 text-white cursor-pointer text-[0.7rem] px-4  min-w-[5rem]  rounded-sm flex items-center gap-1"
        >
          <HiAdjustmentsHorizontal className="w-4 h-4" />
          Filter
        </button>
        <button className="bg-[#9376CA] dark:bg-gray-800 text-white dark:border dark:border-gray-500 cursor-pointer text-[0.7rem] px-4 min-w-[9rem]   rounded-sm flex items-center justify-center gap-2">
          <CiExport className="w-4 h-4" />
          Bulk Export
        </button>
        <button className="bg-[#9376CA] dark:bg-gray-800 dark:border dark:border-gray-500 text-white cursor-pointer text-[0.7rem] px-4  min-w-[8.5rem] rounded-sm flex items-center justify-center gap-2">
      ...
      <div className="rounded-xl mt-5 shadow drop-shadow-xs p-4 sm:p-4 border border-gray-200 dark:border-gray-600">
       ...
       <div className="overflow-x-auto no-scrollbar  ">
          <div
            className="text-[0.7rem] sm:text-[0.8rem]  font-semibold text-black dark:text-gray-50 rounded-md border-y border-gray-100 dark:border-gray-700 bg-[#E4DCF1] dark:bg-gray-900 py-4 px-4 min-h-[44px] "
            style={{
              display: "grid",
              gridTemplateColumns:
          ...
          {filterData.map((emp, index) => (
            <div
              key={index}
              className="text-[0.7rem] sm:text-[0.7.2rem] py-3 px-3 border-b border-gray-100 dark:border-gray-700  dark:text-gray-400 dark:bg-[#A1A1AA]/5"
              style={{
                display: "grid",
                gridTemplateColumns:
                  "60px 90px 1.5fr 1.5fr 1.5fr 1.2fr 1.6fr 50px 50px 50px",
            ...
            >
              ...
              <div className="flex justify-center">
                <img src={ViewIcon} className="w-6 h-6 cursor-pointer" />
              </div>
              <div className="flex justify-center">
                <img src={EditIcon} className="w-6 h-6 cursor-pointer" />
              </div>
              <div className="flex justify-center">
        ...
        {/* Pagination */}
        <div className="flex flex-col sm:flex-row justify-between items-center mt-6 text-xs sm:text-sm text-gray-600 gap-3 flex-wrap">
            ...
            {[1, 2, 3, 4, 5, 6, 7, 8].map((num) => (
              <button
                ...
                className={`px-3 py-1 border rounded ${
                  num === 1
                    ? "bg-[#9376CA] text-white border-[#9376CA]"
        ...
        {/* Footer */}
        <p className="text-black dark:text-gray-400 text-xs sm:text-sm mt-3">
          1-50 of 125
        </p>
      </div>
